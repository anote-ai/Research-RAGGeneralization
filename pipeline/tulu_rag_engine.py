...
# ══════════════════════════════════════════════════════════════
# TULU RAG ENGINE — shared embedding, retrieval & prompt utilities
#
# Imported by tulu_mega_pipeline.py and other pipeline scripts.
# (Formerly "tulu_legal_rag_v8.py" -- renamed since this is now the
# stable shared engine, not a standalone versioned pipeline.)
#
...
# This module lives in pipeline/, with data/ as a sibling folder under the
# same project root (see tulu_mega_pipeline.py's identical DATA_DIR layout).
DATA_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "data")
KANNADA_CORPUS_PATH = os.path.join(DATA_DIR, "kannada_legal_corpus_list.jsonl")
TULU_DATASET_PATH   = os.path.join(DATA_DIR, "tulu_legal_20_consolidated.jsonl")
TULU_KANNADA_CANDIDATE_PATHS = [
    os.path.join(DATA_DIR, "tulu_legal_20_consolidated.jsonl"),
]
...
def load_tulu_queries(path=TULU_DATASET_PATH):
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            romanized = item.get("tulu", item.get("tulu_latin", item.get("tulu_romanized")))
            if romanized is None:
                raise KeyError(f"Entry id={item.get('id')} missing 'tulu'/'tulu_latin'/'tulu_romanized'")
            queries.append({
                "id":           item["id"],
                "tulu":         romanized,
