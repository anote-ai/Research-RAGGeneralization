...
The one RAG-related effect that generalizes across all three models is query language: English-gloss queries under RAG (Condition A) significantly outperform Kannada-script Tulu queries under otherwise identical retrieval (Condition D), for every model (p < .001 in all six comparisons; Figure \ref{fig:query-lang-rag}). A full comparison of Kannada and Romanized script performance across all priming and RAG conditions is reported in Table ~\ref{tab:kannada-romanized}. This suggests that whatever degrades performance under RAG interacts more consistently with query language than with model identity.
...
\bottomrule
\end{tabular}
\caption{Sarvam, Condition H, ids 14 and 17. Row 1: gold and predicted category both Courts/Legal-System, reasoning names the specific recognized term rather than a generic paraphrase. Row 2: gold Land/Property, predicted Housing (gold recovered at rank 3) — the model parses the core action but assigns an adjacent category, indicating partial rather than absent comprehension.}
...
used in this study, with their gold LIST primary and secondary
...
\caption{Full 60-sentence query set: gold LIST categories, English gloss, and Romanized Tulu.}
...
(Kannada script, full priming, no RAG) on query~1 (gold label: Housing) --
...
\textbf{Gold label:} Land/Property \quad \textbf{Predicted:} Consumer/Money/Debt \\
...
\textbf{Gold label:} Housing \quad \textbf{Predicted:} Courts/Legal-System \\
...
screenshots) and {\kannadafont ಬೆರಳಚ್ಚು ಮಾಡಲಾಯಿತು} (a fingerprint
impression was taken). Nothing resembling either detail appears in the
query, which is a roommate/rent dispute with no evidentiary or
court-procedural content at all.
\end{quote}
...
\textit{State v.\ Sudhakar Pai}). The reasoning invents a threat scenario
with no antecedent anywhere in the available context.
\end{quote}
...
\textbf{Gold label:} Torts/Individuals \quad \textbf{Predicted:} Estates/Wills \\
...
\textbf{Query (gloss):} ``My neighbor's dog bit my child while we were walking'' \\
\textbf{Gold label:} Torts/Individuals \quad \textbf{Predicted:} Torts/Individuals \\
...
\textbf{Verification:} The retrieved passage (\textit{Nikitha A.\ v.\ Santhosh Ghatke}, similarity 0.916) is genuinely on-topic Torts/Individuals content. Notably, Llama3 confabulates on this same query under the same condition (see above), predicting Estates/Wills with reasoning citing ``eviction and family members'' - illustrating that retrieval relevance alone does not guarantee correct use of it.
\end{quote}
...
\textbf{Gold label:} Estates/Wills \quad \textbf{Predicted:} Estates/Wills \\
...
inheritance-share dispute between siblings ({\kannadafont ಪಿತ್ರಾಜರ್ಿತ
...
\textbf{Gold label:} Consumer/Money/Debt \quad \textbf{Predicted:} Land/Property \\
...
Ramesh}, similarity 0.911) is a genuinely on-topic financial/debt
dispute, the same case cited in \S\ref{subsec:rag-collapse} as a real, relevant cheque-bounce
matter. Despite retrieval surfacing directly relevant material, the model
still produced an unrelated category. This same pattern, \textit{Sridhar
P.\ v.\ Ramesh} retrieved as the top passage for a genuinely debt-related
query, followed by a Land/Property or Estates/Wills misclassification -
recurred across at least five debt-related queries in this batch (ids 10,
11, 13, 27, 30), suggesting the ``two debt-related queries'' figure cited
in \S\ref{subsec:rag-collapse} is likely an undercount and should be revisited against the
fuller query set.
\end{quote}
...
\bottomrule
\end{tabular}
\caption{Kannada-script vs. Romanized-script comprehension (Hits@1, \%) across all six priming/RAG tiers, per model ($n=60$/condition). Values shown as Kannada\% $>$ Romanized\%. Fisher's exact test, same methodology as Table~\ref{tab:significance-summary}. Computed post-hoc from condition-level results in Table~\ref{tab:rag-results}; no new model calls were made. $^*p<.05$, $^{**}p<.01$, $^{***}p<.001$.}
