...
#
#   3. MuRIL EMBEDDINGS: replaced nomic-embed-text (English-only)
#      with MuRIL (Google's Indic-specific model, trained on Kannada
#      + romanized transliterated text). Falls back to
#      nomic-embed-text if MuRIL is not available locally.
#      Source: Khanuja et al. 2021; literature review Q3.
#
...
KANNADA_CORPUS_PATH = os.path.join(SCRIPT_DIR, "kannada_legal_corpus.jsonl")
...
def get_embedding(text):
    """
    Try MuRIL first (best for Kannada + romanized Indic text per literature review).
    Falls back to nomic-embed-text if MuRIL is not available in Ollama.
    """
    global _embed_model_name
    import ollama

    if _embed_model_name is None:
        for candidate in ["muril", "muril:latest", "nomic-embed-text"]:
            try:
                r = ollama.embed(model=candidate, input="test")
                _embed_model_name = candidate
                print(f"  Embedding model: {candidate}")
                break
            except Exception:
                continue
        if _embed_model_name is None:
            raise RuntimeError("No embedding model available. Pull muril or nomic-embed-text via Ollama.")
...
def mean_center(embedded):
    """
    Fix 4a — mean-centering: subtract corpus mean embedding from all
    chunk vectors. Standard hubness remedy (the 'centering paradigm').
    Suppresses hub documents whose embeddings are close to the corpus
    centroid (like Neelamegham) from dominating all queries.
    """
...
def retrieve(query_text, embedded_corpus, corpus_mean,
    ...
    """
    query_vec = get_embedding(query_text) - corpus_mean
...
def format_retrieved_context(retrieved):
    ...
    for i, p in enumerate(retrieved):
        labels = ", ".join(p.get("categories", [])) or "not annotated"
        ctx += (
            f"\nPassage {i+1} (similarity: {p['score']:.3f}, "
            f"source: {p.get('case_name','?')}, "
            f"CUAD tags on this case: {labels}):\n"
            f"{p['text']}\n"
        )
    return ctx
...
def build_call_prompt(static_prefix, display_text, english_gloss,
    ...
    """
    Fix 5 — query-first prompting: situation appears BEFORE the passages
    and is repeated AFTER them. Explicit instruction not to classify the
    passage content. Each call is independent (stateless) — no prior
    sentences or reflections in context.
    """
    ...
    if english_gloss:
        ...
        situation = (
            f"SITUATION (Tulu written in Kannada script — same script as the "
            f"corpus below): \"{display_text}\"\n"
        ...
        situation = (
            ...
            f"No translation available. Use any patterns you can detect in the "
            f"text and the CUAD tags on retrieved passages to classify."
        )
...
def run_condition(condition_name, label, tulu_queries, embedded_corpus, corpus_mean,
    ...
    for query in tulu_queries:
        ...
        # Show diversity — are we still getting all Neelamegham?
        ...
        prompt = build_call_prompt(
            ...
            script_label, kannada_script_mode
        )
...
def run_rag_pipeline():
    ...
    output_filename = f"tulu_rag_results_v5_{timestamp}.csv"

    print("=" * 65)
    print("TULU LEGAL RAG PIPELINE v5")
